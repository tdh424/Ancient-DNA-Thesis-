...
The following command-line tools must be on `PATH`:
...
`environment.yml` installs all of the tools above from `conda-forge` / `bioconda`.
On Hendrix, load the cluster toolchain before creating or activating the environment:

```bash
module load miniconda/24.5.0 gcc/11.2.0
...
```bash
module load miniconda/24.5.0 gcc/11.2.0  # Hendrix
eval "$(conda shell.bash hook)"
...
All analysis scripts resolve the project root from the checked-out repository location by
default. If needed, override it explicitly with `ANCIENT_DNA_ROOT=/path/to/checkout`.
...
samtools, SPAdes, Mash, PMDtools, pyDamage and the NCBI datasets CLI are installed by the
environment and exposed on `PATH` after activation.
